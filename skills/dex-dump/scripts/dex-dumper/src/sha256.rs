...
fn compress(state: &mut [u32; 8], chunk: &[u8]) {
    ...
    for (index, word) in chunk.chunks_exact(4).take(16).enumerate() {
        schedule[index] = u32::from_be_bytes(word.try_into().expect("four-byte chunk"));
    }
    for index in 16..64 {
...
pub fn digest(data: &[u8]) -> [u8; 32] {
    ...
    let padding_len = if remainder.len() < 56 { 64 } else { 128 };
    let mut tail = vec![0u8; padding_len];
    tail[..remainder.len()].copy_from_slice(remainder);
    tail[remainder.len()] = 0x80;
    let bit_len = (data.len() as u64).wrapping_mul(8);
    tail[padding_len - 8..].copy_from_slice(&bit_len.to_be_bytes());
    for chunk in tail.chunks_exact(64) {
        compress(&mut state, chunk);
    }

    let mut result = [0u8; 32];
    for (destination, word) in result.chunks_exact_mut(4).zip(state) {
