...
- **Formats** — `zip`, `tar` (+`.gz`/`.bz2`/`.xz`), plain `gz`/`bz2`/`xz`, and **`asar`**
  (Electron `app.asar`) with the standard library (portable, no deps). `7z` and `rar`
  shell out to `7z`/`7za`/`7zz`
...
```bash
skillpack run unpack ./suspicious.tgz ./out
skillpack run unpack ./dropper.zip ./out --max-depth 12 --max-bytes 1073741824
```
